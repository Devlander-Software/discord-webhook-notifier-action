...
# Enhanced Discord Notify Action with Message Editing Support
# Features: Smart formatting, retry logic, thread support, mentions, rich embeds, enterprise features, message editing
...
echo "  • Message editing: $([ "$EDIT_MESSAGE" = "true" ] && echo "✅" || echo "❌")"
...
send_or_edit_message() {
    ...
    if [ "$EDIT_MESSAGE" = "true" ]; then
        if [ -z "$MESSAGE_ID" ]; then
            echo "❌ Error: MESSAGE_ID is required when EDIT_MESSAGE is true"
            exit 1
        fi
        
        if [ -z "$BOT_TOKEN" ]; then
            echo "❌ Error: BOT_TOKEN is required when EDIT_MESSAGE is true"
            exit 1
        fi
        
        # Extract channel ID from webhook URL
        CHANNEL_ID=$(echo "$DISCORD_WEBHOOK_URL" | sed -n 's/.*\/channels\/\([0-9]*\)\/.*/\1/p')
        if [ -z "$CHANNEL_ID" ]; then
            echo "❌ Error: Could not extract channel ID from webhook URL"
            exit 1
        fi
        
        echo "📝 Attempt $attempt/$max_attempts: Editing message $MESSAGE_ID..."
    ...
    if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 204 ]; then
        if [ "$EDIT_MESSAGE" = "true" ]; then
            echo "✅ Discord message edited successfully!"
        else
        ...
        if [ "$EDIT_MESSAGE" = "true" ]; then
            echo "❌ Failed to edit Discord message. HTTP Code: $HTTP_CODE"
        else
...
# Main sending/editing logic with retry
attempt=1
while [ $attempt -le "$MAX_RETRIES" ]; do
    if send_or_edit_message $attempt "$MAX_RETRIES"; then
        exit 0
    elif [ $? -eq 2 ]; then
